...
app = Flask(
    __name__,
    static_folder='../frontend',    # serves CSS/JS from frontend/
    template_folder='../frontend',  # looks for HTML in frontend/
)
CORS(app)
...
def analyze_contract():
    try:
        ...
        if not contract_text or len(contract_text) < 100:
            return jsonify({'error': 'Contract text too short (minimum 100 characters)'}), 400
        if len(contract_text) > 500000:
            return jsonify({'error': 'Contract text too long (maximum 500K characters)'}), 400
...
def classify_clause():
    try:
        ...
        predictions = api.quick_classify(text, threshold=threshold)
...
def extract_entities():
    try:
        ...
        if not data or 'text' not in data:
            return jsonify({'error': 'No text provided'}), 400
        text = data['text']
        entities = api.extract_entities(text)
